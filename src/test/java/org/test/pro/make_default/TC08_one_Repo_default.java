...
public class TC08_one_Repo_default extends BaseClass {
...
//TC01,02,03,04,05,06,07,10,11,12		
		...
		WebElement rmveDefault = driver.findElement(By.xpath("//span[contains(text(),'Remove Default')]"));//Locator of Remove Default
		...
		blank();
		System.out.println("TC06 Check if the user login to the application they should be navigated to the default repository");
		...
		blank();
		System.out.println("TC07 Check if the deafult repository set by one user is not applied to another user with the same repository");
		p.getTextEmail().sendKeys("test@email.com");
		p.getTextPass().sendKeys("Abcd@1234");
		p.getSignIn().click();
		...
		refresh();
		blank();
		System.out.println("TC12 Check if any other repository is made default then the default status should be removed for earlier repository");
